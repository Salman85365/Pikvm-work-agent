...
class AgentSettings:
    ...
    max_steps: int = 14
    # Each verified step costs one or more vision calls plus a planner call; a full Slack
    # availability change is five steps, so 180 s was routinely too short once a menu animated
    # slowly or the analyzer escalated.
    max_runtime_seconds: float = 300.0
    min_action_confidence: float = 0.80
    max_no_change_steps: int = 2
    max_repeated_actions: int = 1
    # Verification recovery: an UNCERTAIN verdict first earns one delayed re-observation (the UI
    # may still be animating); a verdict that stays unconfirmed is fed back to the planner, which
    # may choose a different action. The unverified action itself is never re-proposed. Bounded
    # per session.
    verification_reobserve_delay_seconds: float = 1.5
    max_verification_recoveries: int = 2
    ...
    def __post_init__(self) -> None:
        ...
        if not 0 <= self.planner_max_retries <= 5:
            raise AgentConfigurationError("OPENAI_PLANNER_MAX_RETRIES must be between 0 and 5.")
        ...
        if not 0.0 <= self.verification_reobserve_delay_seconds <= 10.0:
            raise AgentConfigurationError(
                "AGENT_VERIFICATION_REOBSERVE_DELAY_SECONDS must be between 0 and 10."
            )
        if not 0 <= self.max_verification_recoveries <= 5:
            raise AgentConfigurationError(
                "AGENT_MAX_VERIFICATION_RECOVERIES must be between 0 and 5."
            )
    ...
    def from_env(cls) -> AgentSettings:
        ...
        try:
            ...
            raise AgentConfigurationError(
                "OPENAI_PLANNER_REASONING_EFFORT must be one of: none, low, medium, high."
            ) from exc
        return cls(
            ...
            max_steps=_env_int("AGENT_MAX_STEPS", 14),
            max_runtime_seconds=_env_float("AGENT_MAX_RUNTIME_SECONDS", 300.0),
            ...
            verification_reobserve_delay_seconds=_env_float(
                "AGENT_VERIFICATION_REOBSERVE_DELAY_SECONDS", 1.5
            ),
            max_verification_recoveries=_env_int("AGENT_MAX_VERIFICATION_RECOVERIES", 2),
